...
class ZmqSocket(object):
    ...
    def recv(self, **kwargs):
        ...
        data = self.sock.recv_multipart(**kwargs)
        LOG.info("\n\n+++++++++++ Received socket type %s\n\n", self.socket_s())
        LOG.info("\n\n+++++++++++ Received data %s\n\n", data[3])
        return data
...
class ConsumerBase(object):
    ...
    def process(self, proxy, ctx, data):
        ...
        LOG.debug("Value of method:%s: ", method)
...
class ZmqBaseReactor(ConsumerBase):
    ...
    def consume_in_thread(self):
        def _consume(sock):
            LOG.info(_("Consuming socket hahaha"))
...
class ZmqProxy(ZmqBaseReactor):
    ...
    def consume(self, sock):
        ...
        if topic.startswith('fanout~'):
            ...
            LOG.info(_("value of topic_proxy ++anant"))
            sock_type = zmq.PUSH
        LOG.info(_("value of topic_proxy ++akash: %s %s"), self.topic_proxy,topic)
        if topic not in self.topic_proxy:
            ...
            def publisher(waiter):
                ...
                try:
                    ...
                    #                     (ipc_dir, topic),
                    #                     sock_type, bind=True)
                    out_sock = ZmqSocket("tcp://127.0.0.1:9000",sock_type, bind=True)
                except RPCException:
                ...
                while(True):
                    ...
                    LOG.debug("Preparation to send halalua. %s" , data[3])
                    out_sock.send(data, copy=False)
                    LOG.debug("Message sent YAY!!!")

            wait_sock_creation = eventlet.event.Event()
            eventlet.spawn(publisher, wait_sock_creation)
            LOG.debug("Preparation to send merry christmas. %s" , data[3])
            try:
                wait_sock_creation.wait()
            except RPCException:
                LOG.debug(_("Topic socket file creation failed."))
                return

        try:
            self.topic_proxy[topic].put_nowait(data)
        except eventlet.queue.Full:
            LOG.debug(_("Local per-topic backlog buffer full for topic "
...
#            (CONF.rpc_zmq_ipc_dir, topic)
        inaddr = "tcp://127.0.0.1:9000"
        LOG.info("Consumer is a zmq.%s",
...
def _call(driver, addr, context, topic, msg, timeout=None,
    ...
    with Timeout(timeout, exception=rpc_common.Timeout):
        try:
            ...
            #)
            msg_waiter = ZmqSocket(
                "tcp://127.0.0.1:9000",zmq.SUB, subscribe=msg_id, bind=False
            )
...
def _multi_send(driver, method, context, topic, msg, timeout=None,
    ...
    LOG.info("Sending message(s) to: %s", queues)
...
def _get_matchmaker(*args, **kwargs):
    ...
    if mm_name in mm_mapping:
        LOG.debug(_LW('rpc_zmq_matchmaker = %(old_val)s is deprecated. '
